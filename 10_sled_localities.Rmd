# Sled dogs: differences across localities
...
```{r alpha_G_div_sled, comment="", message=FALSE, warning=FALSE}
...
**PERMANOVA**
```{r permanova_greenland_q0_sled, comment="", message=FALSE, warning=FALSE}
...
**NMDS**
```{r beta_div_nmds_neutral_plot_G_sled, comment="", message=FALSE, warning=FALSE, fig.height=5, fig.width=10, fig.fullwidth=TRUE}
beta_q0n_dis %>%
...
beta_q1n_dis %>%
  ...
  labs(color='Locality')+ geom_text_repel(aes(label=sample), size=2)
```
...
**PERMANOVA**
```{r permanova_greenland_qP_sled, comment="", message=FALSE, warning=FALSE}
...
betadisper(beta_q1n_distance, sample_metadata_ordered$dog_group) %>% permutest(., pairwise = TRUE) 
anova(betadisper_obj)

adonis2(beta_q1n_distance ~ dog_group, 
        data = sample_metadata_ordered, 
        permutations = 999) %>%
        broom::tidy() %>%
        tt()
pairwise.adonis(beta_q1n_distance, sample_metadata_ordered$dog_group, perm = 999)
```

```{r}
betadisper_obj <- betadisper(beta_q1n_distance, sample_metadata_ordered$dog_group)
dist(betadisper_obj$centroids)
```
...
results_sled <- lapply(region_pairs_sled, function(pair) {
...
save(results_sled, file = "data/ancombc_sled_greenland.Rdata")
```

```{r extract_sled, comment="", message=FALSE}
...
save(ancom_rand_output, 
     file = "data/ancombc_daneborg_others.Rdata")
```

```{r ancom_rand_sled, comment="", message=FALSE, warning=FALSE}
load("data/ancombc_daneborg_others.Rdata")
taxonomy <- data.frame(physeq_genome_green@tax_table) %>%
  rownames_to_column(., "taxon")
...
load(file = "data/ancombc_danenborg_greenland_sp.Rdata")
```
...
save(ancom_rand_output_sp, 
     file = "data/ancombc_danenborg_greenland_sp.Rdata")
```
...
enrichment_code %>%
  ...
  geom_hline(yintercept=0) + 
  coord_flip()+
  geom_text(aes(85, 0.040), label = "Enriched in\n Sirius dogs", color="#666666") +
...
fuso_clr <- genome_counts_clr %>%
  ...
  left_join(
    metadata_clr %>% dplyr::select(sample, locality),
    by = "sample"
  ) 
...
fuso_clr %>% 
  ...
  dplyr::select(locality,Fusobacterium_abundance)

kruskal_test(count ~ locality, data=fuso_clr)

fuso_clr %>%
  wilcox_test(count ~ locality, p.adjust.method = "BH")

fuso_clr %>% 
  group_by(sample) %>% 
  summarise(mean=mean(count), .groups = "drop") %>% 
  left_join(., sample_metadata, by="sample") %>% 
  ggplot(., aes(x = locality, y = mean, fill = locality)) +
  geom_boxplot(outlier.shape = NA, alpha = 0.7) +
