...
sample_metadata <- read_csv("data/sample_metadata.csv") %>% 
  ...
  select(sample, breed, origin, country, locality)

metadata_new1 <- read_csv("data/PRJEB85799.csv") %>% 
  rename(country=notes) %>% 
  rename(sample=name)

sample_metadata <- read_csv("data/meta_new.csv") %>% 
  select(-"Research agreement",-"Owner age range") %>% 
  rename(breed=Breed) %>% 
  rename(origin="Origin of the dog") %>% 
  rename(locality=District) %>% 
  rename(sample=Code)%>% 
  left_join(., metadata_new1[c(3,4)], by="sample")%>% 
  select(sample, breed, origin, country, locality) %>% 
  bind_rows(sample_metadata)
```
...
genome_features <- read_tsv("data/quality_report.tsv") %>% 
  rename(genome=1) %>% 
...
genome_gifts <- distill(genome_annotations,GIFT_db,genomecol=1,annotcol=c(6,8,9), verbosity=TRUE)
