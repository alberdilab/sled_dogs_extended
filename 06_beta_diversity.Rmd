...
Significant
```{r}
pair_results <- pairwise.adonis(beta_q1n_dis, sample_metadata_ordered$locality, perm = 999)
pair_results %>%
  filter(p.adjusted < 0.05)
```
...
Significant
```{r}
pair_results <- pairwise.adonis(beta_q1p_dis, sample_metadata_ordered$locality, perm = 999)
pair_results %>%
  filter(p.adjusted < 0.05)
```
...
#### Neutral
***Country***
```{r beta_div_nmds_neutral_country, comment="", message=FALSE, warning=FALSE, fig.height=7, fig.width=12, fig.fullwidth=TRUE}
beta_q1n_dis %>%
  vegan::metaMDS(., trymax = 500, k = 2, trace=0) %>%
  vegan::scores() %>%
  as_tibble(., rownames = "sample") %>%
...
```
```{r pcoa_neutral_type, comment="", message=FALSE, warning=FALSE, fig.height=7, fig.width=10, fig.fullwidth=TRUE}
...
```{r centroid_distances_locality_figure, comment="", message=FALSE, warning=FALSE, fig.height=3, fig.width=6, fig.fullwidth=TRUE}
