...
```{r gitfs_functional_country, echo=TRUE,results=TRUE}
...
```{r final_plot_sled_dom, comment="", message=FALSE, warning=FALSE, fig.height=8, fig.width=12, fig.fullwidth=TRUE}
enrichment_code %>%
...
```{r final_plot_sled_domest, comment="", message=FALSE, warning=FALSE, fig.height=8, fig.width=12, fig.fullwidth=TRUE}
enrichment_code %>%
  left_join(gift_colors[c(1,3,4)], by=join_by(Code_function==Code_function))%>%
  ggplot(aes(x=forcats::fct_reorder(Function,log2FC), y=log2FC, fill=legend)) + 
  geom_col() +
